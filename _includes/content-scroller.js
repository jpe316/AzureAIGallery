...
var exampleItemsLength = $('.exampleitem').length;
var exampleItemSize = $('.exampleitem').outerWidth(true);
// get some relevant size for the paddle triggering point
var paddleMargin = 20;

// get wrapper width
var getExampleMenuWrapperSize = function() {
	return $('#example-wrapper').outerWidth();
}
...
var getExampleMenuPosition = function() {
	return $('#example-menu').scrollLeft();
};

// finally, what happens when we are actually scrolling the menu
$('#example-menu').on('scroll', function() {
...
$(rightPaddle_examples).on('click', function() {
	$('#example-menu').animate( { scrollLeft: exampleMenuInvisibleSize}, scrollDuration);
});

// scroll to right
$(leftPaddle_examples).on('click', function() {
	$('#example-menu').animate( { scrollLeft: '0' }, scrollDuration);
});
...
// get items dimensions
var modelItemsLength = $('.modelitem').length;
var modelItemSize = $('.modelitem').outerWidth(true);


// get wrapper width
var getModelMenuWrapperSize = function() {
	return $('#model-wrapper').outerWidth();
}
...
var getModelMenuPosition = function() {
	return $('#model-menu').scrollLeft();
};

// finally, what happens when we are actually scrolling the menu
$('#model-menu').on('scroll', function() {
...
$(rightPaddle_models).on('click', function() {
	$('#model-menu').animate( { scrollLeft: modelMenuInvisibleSize}, scrollDuration);
});

// scroll to right
$(leftPaddle_models).on('click', function() {
	$('#model-menu').animate( { scrollLeft: '0' }, scrollDuration);
});